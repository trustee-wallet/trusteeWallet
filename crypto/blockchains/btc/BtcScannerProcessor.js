...
export default class BtcScannerProcessor {
     ...
     * @private
     */
    _trezorPathXpub = 'https://btc1.trezor.io/api/v2/xpub/'
    ...
    async _get(address, additionalData) {
        ...
        if (prefix === 'xpub' || prefix === 'zpub') {
            link = this._trezorPathXpub + address + '?details=txs&gap=9999'
        }
    ...
    async getBalance(address) {
        ...
        return { balance: res.balance, unconfirmed: res.unconfirmedBalance, provider: 'trezor', addresses : res.addresses }
    }
