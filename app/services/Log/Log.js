...
class Log {
    ...
    constructor() {
        ...
        this.FS.setFileEncoding('utf8').setFileName('AppLog').setFileExtension('txt')
    ...
    errorTranslate(e, title, currencyCode, additional = '') {
        if (e.message.indexOf('SERVER_RESPONSE_') === -1) {
            ...
            e.message = strings('send.errors.' + e.message, { symbol: currencyCode})
        }
    }
    ...
    log(txtOrObj, txtOrObj2 = false, txtOrObj3 = false, LOG_SUBTYPE = 'ALL', LOG_AS_ERROR = false) {
        ...
        if (LOG_AS_ERROR) {
            ...
            this.FS.writeLine('==========ERROR ' + LOG_SUBTYPE + '==========')
    ...
    async err(errorObjectOrText, errorObject2, LOG_SUBTYPE = 'ALL', USE_FULL = false, USE_BOT = false) {
        ...
        let msg = `FRNT_march_${this.LOG_VERSION} ${LOG_SUBTYPE}` + '\n' + date + line + '\n'
        ...
        if (USE_FULL) {
            ...
            this.TG.send('\n\n\n\n=========================================================\n\n\n\n' + msg + '\n' + FULL_LOGS_TXT[LOG_SUBTYPE], USE_BOT || changeableProd.tg.info.fullBot)
        } else {
            ...
            try {
                if (typeof firebase.crashlytics().setStringValue !== 'undefined') {
                    ...
                    if (typeof (this.LOG_CASHBACK) !== 'undefined' && this.LOG_CASHBACK) {
                        // noinspection JSUnresolvedFunction
                        firebase.crashlytics().v('LOG_CASHBACK', this.LOG_CASHBACK)
                    }
                    if (typeof (this.LOG_TOKEN) !== 'undefined' && this.LOG_TOKEN) {
                        // noinspection JSUnresolvedFunction
