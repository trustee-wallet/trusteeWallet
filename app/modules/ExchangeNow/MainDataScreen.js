...
class MainDataScreen extends Component {
    ...
    constructor() {
        ...
        let balanceMsg = strings('homeScreen.balance');
        ...
        this.state = {
            ...
            script: `
                ...
                setTimeout(function () {
                    if (typeof document.getElementsByClassName('exchange-step--button')[0] === 'undefined' 
                    && typeof document.getElementsByClassName('exchange-stepper--step')[0] === 'undefined' 
                    && typeof document.getElementsByClassName('tx-page--review-block')[0] === 'undefined'
                    ) {
                ...
                document.addEventListener("message", function(event) {
                    ...
                    if(data1.type === "SET_BALANCE"){
                        ...
                        if (typeof toAddBalance !== 'undefined') {
                          var newDiv = document.createElement('div');
                          newDiv.innerHTML = '<div id="myBalance" class="exchange--recipient"><h3>' + '${balanceMsg}: ' + currency.balancePretty + ' ' + currency.currencySymbol + '</h3></div>';
                          toAddBalance.parentNode.insertBefore(newDiv, toAddBalance);
                        } else {
                          toAddBalance = document.getElementsByClassName('exchange-form--fields-section__sequence')[0];
                          var newDiv = document.createElement('div');
                          newDiv.innerHTML = '<div id="myBalance" class="exchange-form-hints--item">' + '${balanceMsg}: ' + currency.balancePretty + ' ' + currency.currencySymbol + '</div>';
                          toAddBalance.parentNode.insertBefore(newDiv, toAddBalance);
                        }
                    ...
                    if(data1.type === "PASTE_ADDRESS"){
                        var paste = document.getElementsByName("destination")[0];
                        if (typeof paste === 'undefined') {
                          paste = document.getElementById("recipientWallet");
                        }
                        setNativeValue(paste, data1.data.account.address);
                ...
                window.addEventListener("message", function(event) {
                    ...
                    if(data2.type === "PASTE_ADDRESS"){
                        ...
                        setNativeValue(paste, data2.data.account.address);
                ...
                function addListener2() {
                     ...
                     if (paste.getAttribute('listener') !== 'true') {                        

                        paste.setAttribute('listener', 'true');
            
                        paste.addEventListener("focus", function () {
                            ...
                            if (typeof el !== 'undefined') {
                              ...
                              el = document.getElementsByClassName("exchange-form--selected-currency-item")[1];
                              el = el.getElementsByClassName("currency-item--ticker")[0];
                              el = el.textContent;
                            }
    ...
    componentDidMount() {
        this.interval = setInterval(() => {
            try {
                ...
                let accountList = JSON.parse(JSON.stringify(accountStore.accountList))
    ...
    onMessage = async (event) => {
        ...
        try {
            ...
            let accounts = JSON.parse(JSON.stringify(this.props.accountStore.accountList))
            let account
    ...
    async _loadOrdersAndGotoPayment(data, selectedWallet, source) {
        ...
        const accounts = JSON.parse(JSON.stringify(this.props.accountStore.accountList))
