...
const renderSynchronization = () => {
    ...
    return (
        <View style={styles.cryptoList__syncRow}>
            <Text style={[styles.cryptoList__text, { marginRight: 5, color: colors.common.text2 }]}>{strings('homeScreen.synchronizing')}</Text>
            <View>{Platform.OS === 'ios' ? <UIActivityIndicator size={10} color='#404040' /> : <MaterialIndicator size={10} color='#404040' />}</View>
        </View>
    )
}
    ...
    return (
        <GradientView
            ...
            {props.type === 'NFT' ? (
                <View style={styles.cryptoList__info}>
                    <View style={styles.cryptoList__currency__balance}>
                        <Text style={[styles.cryptoList__title, { color: colors.common.text1 }]}>{cryptoCurrency.currencySymbol}</Text>
                    </View>

                    <View style={styles.cryptoList__currency__rate}>
                        <Text style={[styles.cryptoList__text, { color: colors.common.text2 }]}>{cryptoCurrency.currencyName}</Text>
                    </View>
                </View>
            ) : (
                <View style={styles.cryptoList__info}>
                    <View style={styles.cryptoList__currency__balance}>
                        <View style={styles.stakingPercent}>
                            <Text style={[styles.cryptoList__title, { color: colors.common.text1 }]}>{cryptoCurrency.currencySymbol}</Text>
                            {availableStaking ? <PercentView value={stakingCoins[currencyCode]} staking containerStyle={{ marginTop: -2 }} /> : null}
                        </View>
                        {typeof isSynchronized !== 'undefined' ? (
                            !isSynchronized ? (
                                ...
                                <Text style={[styles.cryptoList__title, { color: colors.common.text1 }]}>
                                    {BlocksoftPrettyNumbers.makeCut(account.balancePretty, 5, 'WalletList', false).separated}
                                </Text>
                            ) : (
                        ...
                        <View style={[styles.cryptoList__currency__changes, { borderColor: colors.homeScreen.listItemSeparatorLine }]}>
                            <View style={styles.cryptoList__currency__changes__rate}>
                                {priceChangePercentage24h !== null && priceChangePercentage24h !== undefined && priceChangePercentage24h !== 0 && (
                                    <CustomIcon
                                        name={priceChangePercentage24h > 0 ? 'rateUp' : 'rateDown'}
            ...
            {!props.constructorMode ? null : (
                <TouchableDebounce style={{ marginLeft: GRID_SIZE }} activeOpacity={0.7} onPressIn={props.onDrag} hitSlop={HIT_SLOP}>
