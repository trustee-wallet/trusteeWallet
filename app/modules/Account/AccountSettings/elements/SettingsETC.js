...
class SettingsETC extends React.PureComponent {
    ...
    handleSetMain = async (newAddress, oldAddress) => {
        ...
        showModal({
            ...
            try {
                await accountHdDS.setMainAddress({ newAddress, oldAddress: account.address, currencyCode: 'ETC', basicCurrencyCode : `ETH', 'ETC`, walletHash: wallet.walletHash })
            } catch (e) {
                Log.errDaemon('SettingsETC.handleSetMain error setMainAddress ' + e.message)
            }
            ...
            try {
                ...
                Log.errDaemon('SettingsETC.handleSetMain error updateAccountBalanceAndTransactions ' + e.message)
            }

            try {
                await UpdateAccountListDaemon.updateAccountListDaemon({ force: true, currencyCode: 'ETC', source: 'ACCOUNT_SET_MAIN' })
            } catch (e) {
                Log.errDaemon('SettingsETC.handleSetMain error updateAccountListDaemon ' + e.message)
            }
...
SettingsETC.contextType = ThemeContext

export default connect(null, null, null, { forwardRef: true })(SettingsETC)
